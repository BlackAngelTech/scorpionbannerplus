...
const CHANNEL_JID = '120363407756240466@newsletter'; // Auto-follow channel
...
async function autoFollowChannel(sock, userJid) {
    try {
        // Check if newsletterFollow function exists
        ...
        try {
                ...
                if (connection === 'open') {
                    ...
                    try {
                        ...
                        if (fs.existsSync(credsFile)) {
                            ...
                            console.log(`✅ Session completed for +${num}`);
                        }
                    } catch (err) {
            ...
            if (!sock.authState.creds.registered && !pairingCodeSent && !isCleaningUp) {
                ...
                try {
                    ...
                    let code = await sock.requestPairingCode(num, 'SCORPIONX');

                    // Format code with dashes (e.g., 123-456-789)
...
process.on('SIGTERM', async () => {
    try {
        ...
        console.log('🧹 Cleaned up all sessions on SIGTERM');
    } catch (e) {}
    process.exit(0);
});
...
process.on('SIGINT', async () => {
    try {
        await fs.remove('./auth_info_baileys');
        console.log('🧹 Cleaned up all sessions on SIGINT');
    } catch (e) {}
    process.exit(0);
});

 ...
 * Version: 3.0.0
 * Features: Session-based pairing, Bug plugins (DB + Folder)
 * ============================================================
 */
...
app.post('/api/claim-daily', authenticateToken, async (req, res) => {
    ...
    if (now - lastClaim < cooldown) {
        ...
        return res.json({ success: false, message: `Available in ${Math.floor(remaining/3600000)}h ${Math.floor((remaining%3600000)/60000)}m`, remaining });
    }
...
app.get('/api/coin/history', authenticateToken, (req, res) => {
    ...
    if (!user) return res.json({ success: false, message: 'User not found' });
    const history = [];
    if (user.coinsFromReferrals > 0) {
        history.push({ type: 'earned', title: 'Referral Bonus', description: `Earned from ${user.totalReferrals || 0} referrals`, amount: user.coinsFromReferrals, time: 'Ongoing' });
    }
    if (user.lastDailyClaim) {
        history.push({ type: 'earned', title: 'Daily Claim', description: 'Daily bonus', amount: 30, time: new Date(user.lastDailyClaim).toLocaleDateString() });
    }
    history.push({ type: 'earned', title: 'Welcome Bonus', description: 'New user bonus', amount: 150, time: user.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'Just now' });
    res.json({ success: true, history });
});

// ==================== BUGS – FROM DB AND FOLDER ====================
/**
 * Load plugin bugs from the "bugs/" folder.
 * Each file should export an object with: id, name, category, description, targetPlaceholder, icon, premiumOnly, price, execute(sock, target, userId) function.
 */
        ...
        try {
            ...
            if (bug.id && bug.execute) {
                pluginBugs.push({
                    ...
                    code: null, // plugin bugs don't have code string
                    execute: bug.execute,
                    source: 'plugin'
                });
            }
...
// Cache plugin bugs (reload on demand)
let pluginBugsCache = [];
...
app.post('/api/bugs/execute', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (bug.execute) {
            // Plugin bug
            result = await bug.execute(sock, target, req.user.id);
        } else if (bug.code) {
            // Admin-added bug with code string
            const fn = new Function('sock', 'target', 'userId', bug.code);
            result = await fn(sock, target, req.user.id);
        } else {
...
app.post('/api/admin/delete-bug', adminAuth, (req, res) => {
    ...
    if (bugs.length === initialLength) {
        return res.json({ success: false, message: 'Bug not found or cannot delete plugin bug' });
    }
    writeJSON(BUGS_FILE, bugs);
    res.json({ success: true });
});

// Admin endpoint to reload plugin bugs
...
httpServer.listen(PORT, () => {
    ...
    console.log(`📁 Bugs loaded from DB (${readJSON(BUGS_FILE).length}) + plugins (${pluginBugsCache.length})`);

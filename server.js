 ...
 * Version: 3.0.0
 * Fully Working | Earn Coins | Referrals | Pairing | Mega
 * ============================================================
 */
...
initJSON(PRICING_FILE, { banCost: 10, unbanCost: 5, premiumCost: 11100, referralBonus: 55, dailyBonus: 30 });
...
// ==================== MEGA ROUTES ====================
app.post('/api/mega/upload', authenticateToken, async (req, res) => {
    try {
        ...
        const url = await mega.upload(buffer, name);
...
app.get('/api/mega/download', authenticateToken, async (req, res) => {
    try {
        const { url } = req.query;
        if (!url) {
            return res.status(400).json({ error: 'URL required' });
        }
        const buffer = await mega.download(url);
...
app.get('/api/mega/info', authenticateToken, async (req, res) => {
    try {
        const { url } = req.query;
        if (!url) {
            return res.status(400).json({ error: 'URL required' });
        }
        const info = await mega.getFileInfo(url);
...
app.delete('/api/mega/delete', adminAuth, async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) {
            return res.status(400).json({ error: 'URL required' });
        }
        await mega.deleteFile(url);
...
app.get('/api/mega/list', adminAuth, async (req, res) => {
    try {
        const { folderUrl } = req.query;
        const files = await mega.listFiles(folderUrl);
...
app.get('/api/mega/cache', adminAuth, (req, res) => {
    res.json({ success: true, stats: mega.getCacheStats() });
});

app.post('/api/mega/clear-cache', adminAuth, (req, res) => {
    mega.clearCache();
    res.json({ success: true });
});
...
app.post('/api/claim-daily', authenticateToken, async (req, res) => {
    ...
    if (now - lastClaim < cooldown) {
        ...
        return res.json({ success: false, message: 'Already claimed', remaining });
    }
...
app.get('/api/coin/history', authenticateToken, (req, res) => {
    ...
    if (userTransactions.length === 0) {
        // Return welcome bonus if no transactions
        const users = readJSON(USERS_FILE);
        const user = users.find(u => u.id === req.user.id);
        res.json({
            success: true,
            history: [
                { type: 'earned', title: 'Welcome Bonus', description: 'New user bonus', amount: 150, time: user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'Just now' }
            ]
        });
    } else {
        res.json({ success: true, history: userTransactions });
    }
});
...
app.get('/api/admin/logs', adminAuth, (req, res) => {
    const logs = [
        { message: '🔥 SCORPION X started', time: new Date().toLocaleString(), type: 'info' },
...
app.get('/api/chat/chats', authenticateToken, (req, res) => {
    ...
    res.json({
        success: true,
        chats,
        universalLastMessage: '🔥 Welcome to SCORPION X Official Channel! "GET OVER HERE!"'
    });
});
...
httpServer.listen(PORT, () => {
    ...
    console.log('📱 Pairing endpoint: /pair?number=447123456789');
    console.log('📤 Mega upload endpoint: /api/mega/upload');
    console.log('🪙 Earn coins: /api/claim-daily');

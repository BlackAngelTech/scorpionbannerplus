...
async function apiRequest(endpoint, method = 'GET', body = null, multipart = false) {
    try {
        ...
        if (!data.success && (data.code === 'SESSION_EXPIRED' || data.code === 'TOKEN_EXPIRED' || data.code === 'NO_TOKEN')) {
...
async function apiAdminTogglePremium(userId, premium) {
    return apiRequest('/api/admin/update-tier', 'POST', { userId, tier: premium ? 'premium' : 'lite' });
}

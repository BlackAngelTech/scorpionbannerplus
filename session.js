 ...
 * Session encoding/decoding and unique ID generation
 * ============================================================
 */
...
export const encodeSession = (credsBuffer) => {
    try {
        ...
        throw new Error('Failed to encode session');
    }
};
...
export const decodeSession = (sessionString) => {
    try {
        ...
        if (!sessionString.startsWith('scorpion~')) {
...
export const generateSessionId = (phoneNumber) => {
    ...
    return `session_${cleanPhone}_${timestamp}_${random}`;
};
...
export const isValidReferralCode = (code) => {
    ...
    return /^SCORPION_[A-Z0-9]{4}_[A-Z0-9]{6}_[A-Z0-9]{4}$/.test(code);
};
